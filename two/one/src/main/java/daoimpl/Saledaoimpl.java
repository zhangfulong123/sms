...
public class Saledaoimpl implements Saledao {
	...
	public String updatesal(int fid,int num) {
		this.db=new DBUtil();
		String sql="update sale set num=? where fid="+fid;
		try {
			int i = this.db.update(sql, num);
			return i>0?"成功":"失败";
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public Sale selectsal(int fid) {
		this.db=new DBUtil();
		String sql="select * from sale where fid="+fid;
		try {
			ResultSet re = this.db.query(sql);
			if(re.next()){
				return new Sale(re.getInt("fid"), re.getInt("num"));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
